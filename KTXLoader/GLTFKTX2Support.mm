...
BOOL GLTFMetalDeviceSupportsETC(id<MTLDevice> device) {
...
BOOL GLTFMetalDeviceSupportsASTC(id<MTLDevice> device) {
    if (@available(macos 10.15, iOS 13.0, *)) {
        return [device supportsFamily:MTLGPUFamilyApple8] ||
               [device supportsFamily:MTLGPUFamilyApple7] ||
               [device supportsFamily:MTLGPUFamilyApple6] ||
...
BOOL GLTFMetalDeviceSupportsBC(id<MTLDevice> device) {
...
id<MTLTexture> GLTFCreateTextureFromKTX2Data(NSData *data, id<MTLDevice> device) {
    ...
    result = ktxTexture2_CreateFromMemory(bytes, size, flags, &ktx2Texture);
